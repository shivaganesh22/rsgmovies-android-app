...
class OpenFolder extends StatefulWidget {
  ...
  const OpenFolder({Key? key, required this.id}) : super(key: key);
...
class _OpenFolderState extends State<OpenFolder> {
  ...
  Future<void> apicall() async {
    if (await AuthService.isLogged()) {
      ...
      http.Response res = await http.post(
        Uri.parse('https://rsg-movies.vercel.app/api/open/${widget.id}'),
  ...
  Future<void> playFolder(id) async {
    ...
    if (await AuthService.isLogged()) {
      ...
      if (res.statusCode == 200) {
        ...
        context.push(context.namedLocation("player",pathParameters: {"data":res.body}));
      } else{
        EasyLoading.dismiss();
  ...
  Future<void> editFolder(name,id) async {
    ...
    if (await AuthService.isLogged()) {
      Map<String, String> user = await AuthService.getLoginDetails();
      http.Response res = await http.post(
        Uri.parse('https://rsg-movies.vercel.app/api/rename/folder/${id}/?name=${name}'),
        body: {"email": user["email"]!, "password": user["password"]!},
      );
      apicall();
  ...
  Future<void> editFile(name,id) async {
    ...
    if (await AuthService.isLogged()) {
      Map<String, String> user = await AuthService.getLoginDetails();
      http.Response res = await http.post(
        Uri.parse('https://rsg-movies.vercel.app/api/rename/file/${id}/?name=${name}'),
        body: {"email": user["email"]!, "password": user["password"]!},
      );
      apicall();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Open Folder")),
                        ...
                        return Column(
                          children: [
                            ListTile(
                              ...
                              subtitle: Text(torrents[index]['warnings'] ==
                                      '[]' 
                                  ? '':
                                  torrents[index]['warnings'].toString(),textAlign: TextAlign.center,style: TextStyle(color:Colors.red),),
                            ),
                            Row(
                            ...
                            children: [
                              ListTile(
                                title: isEditing[index]
                                    ? TextField(
                                        controller: TextEditingController(text:folders[index]['name']),
                                        onSubmitted: (text) {
                                          editFolder(text,folders[index]['id']);
                                        },
                                      )
                                    : Text(
                                        folders[index]["name"],
                          ...
                          children: [
                            ListTile(
                               title: isEditingFile[index]
                                    ? TextField(
                                        controller: TextEditingController(text:files[index]['name']),
                                        onSubmitted: (text) {
                                          editFile(text,files[index]['folder_file_id']);
                                        },
                                      )
                                    : Text(
                                        files[index]["name"],
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                IconButton(
                                  ...
                                  onPressed: () {
                                    downloadFile(files[index]['folder_file_id']);
                                  },
                                ),
                                IconButton(
                                    ...
                                    onPressed: () {
                                     setState(() {
                                      isEditingFile[index] = !isEditingFile[index];
                                ...
                                IconButton(
                                  ...
                                  onPressed: () {
                                    deleteFile(files[index]['folder_file_id']);
                                  },
                                ),
                              ],
                            ),
